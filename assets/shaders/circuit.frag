...
uniform float uParam1;   // 6 - Trace Length
uniform float uParam2;   // 7 - Density
...
void main() {
    ...
    float spacing = uParam1 > 0.1 ? uParam1 * uWidth * 5.0 : uWidth * 10.0;
    float count = round(perimeter / spacing);
    float exactSpacing = perimeter / count;
    
    float animU = u + uTime * 20.0; // Flow direction
    
    float cell = floor(animU / exactSpacing);
    float localU = fract(animU / exactSpacing);
    
    // Circuit pattern: 
    // Random length for each trace
    float noise = fract(sin(cell * 12.9898) * 43758.5453);
    float traceLen = 0.3 + 0.6 * noise; 
    
    float traceMask = step(1.0 - traceLen, localU);
    
    // Add "Dot" at the head of the trace
    float dotPos = 1.0 - traceLen;
    float distToDot = abs(localU - dotPos) * exactSpacing;
    float dotMask = 1.0 - smoothstep(uWidth * 0.4, uWidth * 0.4 + 1.0, distToDot);
    
    float circuit = max(traceMask, dotMask);
    ...
    if (dotMask > 0.5) activeColor = mix(activeColor, vec3(1.0), 0.5); // Dots are brighter
    ...
    if (uGlow > 0.0) {
        ...
        glowFactor = exp(-falloff * dist * dist) * uGlow * 2.0 * circuit;
    }

    vec3 finalColor = vec3(0.0);
    float finalAlpha = 0.0;

    finalColor += uColor2 * coreMask; // Circuit traces on background? No, bg is behind
    // Let's draw BG
