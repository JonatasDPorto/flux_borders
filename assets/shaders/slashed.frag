...
void main() {
    ...
    float spacingMult = uParam1 > 0.1 ? uParam1 : 3.0;
    ...
    float ratio = uParam2 > 0.0 ? uParam2 : 0.5;
    float pattern = fract(animU / exactSpacing);
    float dash = step(1.0 - ratio, pattern);
    ...
    if (uGlow > 0.0) {
        ...
        glowFactor = exp(-falloff * dist * dist) * uGlow * 2.0 * dash;
    }
    ...
    finalColor = mix(finalColor, activeColor, coreMask * dash);
    finalAlpha = max(finalAlpha, coreMask * dash);

    vec3 renderedGlow = activeColor * glowFactor;
    float glowAlpha = clamp(glowFactor, 0.0, 1.0);
    
    finalColor = max(finalColor, renderedGlow);
    finalAlpha = max(finalAlpha, max(coreMask * dash, glowAlpha * 0.5));
